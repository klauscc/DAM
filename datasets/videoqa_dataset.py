...
class VideoQA_Dataset(Dataset):
    ...
    def _get_video(self, video_id):
        if video_id not in self.features:
            print(f"video {video_id} don't have features")
        ...
        if len(video) > self.max_feats:
            ...
            video_len = len(video)
            video = th.cat([video, th.zeros(self.max_feats - video_len, self.features_dim)], 0)
        else:
            video_len = self.max_feats
...
def videoqa_collate_fn(batch):
    ...
    text = (
        ...
        else [[batch[i]["text"][j] for i in range(bs)] for j in range(len(batch[0]["text"]))]
    )
...
def build_videoqa_dataset(dataset_name, split, args, tokenizer):
    if dataset_name == "msvd":
        ...
        if split == "train":
            csv_path = args.tgif_train_csv_path
        elif split == "val":
            csv_path = args.tgif_test_csv_path  # no val set in TGIF
        elif split == "test":
            csv_path = args.tgif_test_csv_path
        else:
            raise NotImplementedError
    ...
    elif dataset_name == "envqa":
        if split == "train":
            csv_path = args.envqa_train_csv_path
        elif split == "val":
            csv_path = args.envqa_val_csv_path  # no val set in envqa
        elif split == "test":
            csv_path = args.envqa_test_csv_path
        else:
            raise NotImplementedError
        features_path = args.envqa_features_path
        vocab_path = args.envqa_vocab_path
        type_map = None
        subtitles_path = None
    elif dataset_name == "trafficqa":
        if split == "train":
            csv_path = args.trafficqa_train_csv_path
        elif split == "val":
            csv_path = args.trafficqa_test_csv_path  # no val set in trafficqa
        elif split == "test":
            csv_path = args.trafficqa_test_csv_path
        else:
            raise NotImplementedError
        features_path = args.trafficqa_features_path
        vocab_path = args.trafficqa_vocab_path
        type_map = None
        subtitles_path = None
        ...
        if split == "train":
            ...
            csv_path = args.agqa_test_csv_path  # no val set in agqa
        elif split == "test":
    ...
    # if args.unified_vocab_path is specified, use this one.
    if args.unified_vocab_path != "":
        vocab_path = args.unified_vocab_path
        print(f"USE VOCABULARY: {vocab_path}")
    ...
    if dataset_name in [
        ...
        return VideoQA_Dataset(
            ...
            use_context=(args.use_context and dataset_name != "tgif"),  # no speech in GIFs
