...
from model.merge_model.merge_adapter import Merge_Linear
from model.merge_model.merge_adapter import MergeAdapter as Adapter
    ...
    def __init__(self, config, ds_factor, dropout):
        ...
        if self.ds_factor:
            self.adapter = Adapter(
                ds_factor, config.hidden_size, dropout=dropout, num_models=config.num_models
            )

    def forward(self, hidden_states, input_tensor, prob):
        hidden_states = self.dense(hidden_states)
        if self.ds_factor:
            hidden_states = self.adapter(hidden_states, prob)
...
class DebertaV2Attention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(self_output, query_states, prob)
...
class DebertaV2Layer(nn.Module):
    ...
    def forward(
        ...
        layer_output = self.output(intermediate_output, attention_output, prob)
        if return_att:
...
def make_log_bucket_position(relative_pos, bucket_size, max_position):
    ...
    log_pos = (
        np.ceil(np.log(abs_pos / mid) / np.log((max_position - 1) / mid) * (mid - 1)) + mid
    )
...
class DebertaV2Embeddings(nn.Module):
    ...
    def __init__(
        ...
        if self.features_dim:
            ...
            self.linear_video = Merge_Linear(
                features_dim, config.hidden_size, config.num_models
            )

    def get_video_embedding(self, video, prob):
        video = self.linear_video(video, prob)
        return video
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if self.features_dim and video is not None:
                video = self.get_video_embedding(video, prob)
...
class DebertaV2ForMaskedLM(DebertaV2PreTrainedModel):
    ...
    def __init__(
        ...
        assert config.num_models > 0, "num_models must be set when merging"
    ...
    def emd_context_layer(self, encoder_layers, z_states, attention_mask, encoder, prob):
        if attention_mask.dim() <= 2:
            extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
...
class DebertaV2LMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states, embedding_weight, bias=None):
        ...
        hidden_states = self.LayerNorm(hidden_states)
        if bias is not None:
            logits = torch.matmul(hidden_states, embedding_weight.t().to(hidden_states)) + bias
        else:
            logits = (
                torch.matmul(hidden_states, embedding_weight.t().to(hidden_states)) + self.bias
            )
        return logits
